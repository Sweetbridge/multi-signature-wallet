...
contract('MultiSigWalletWithDailyLimit', (accounts) => {
  ...
  it('create multisig', async () => {
    ...
    let owner2Balance = await utils.balanceOf(web3, accounts[1])
    await multisigInstance.submitTransaction(accounts[0], value1, "", {from: accounts[1]})
    assert.ok(
      owner2Balance.sub((await utils.balanceOf(web3, accounts[1]))).toNumber() < 1e17
    )
    assert.equal(
      owner1Balance.add(value1).toNumber(),
      (await utils.balanceOf(web3, accounts[0])).toNumber()
    )
    assert.equal(
      balance.sub(value1).toNumber(),
      (await utils.balanceOf(web3, multisigInstance.address)).toNumber()
    )
    ...
    let hasNotExpired = await multisigInstance.hasNotExpired(transactionIdFailed)
    assert.ok(hasNotExpired)
